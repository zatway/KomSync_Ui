    ...
    return (
        <div className='container mx-auto py-6 sm:py-8 px-3 sm:px-4 max-w-4xl space-y-6'>
            <div className='flex flex-wrap items-center gap-2 sm:gap-3'>
                <Button variant='ghost' size="sm" className="px-2" onClick={() => navigate(`${AppRoutes.TASKS}/${projectId}`)}>
                    <ArrowLeft className='mr-2 h-4 w-4' />К доске
                </Button>
                <Button variant='outline' size="sm" onClick={() => navigate(`${AppRoutes.TASKS}/${projectId}/edit/${taskId}`)}>
            ...
            <Card>
                <CardHeader><CardTitle className='text-lg'>Комментарии</CardTitle></CardHeader>
                    ...
                    <ul className='space-y-3'>
                        {task.comments.map((c) => (
                            <li key={c.id} className='border rounded-md p-3 text-sm'>
                                <div className='font-medium flex items-center justify-between gap-2 flex-wrap'>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {currentUserId === c.userId && (
                                            <>
                                                {editingId === c.id ? (
                                                    <Button type="button" size="sm" variant="secondary" disabled={updatingComment} onClick={() => void saveEdit()}>
                                                        Сохранить
                                                    </Button>
                                                ) : (
                                                    <Button type="button" size="sm" variant="ghost" onClick={() => startEdit(c.id, c.content)}>
                                                        Правка
                                                    </Button>
                                                )}
                                                <Button type="button" size="sm" variant="ghost" className="text-destructive" disabled={deletingComment} onClick={() => void removeComment(c.id)}>
                                                    Удалить
                                                </Button>
                                            </>
                                        )}
                                ...
                                {editingId === c.id ? (
                                    <Textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} rows={3} className="mt-2" />
                                ) : (
                                ...
                                {c.attachments?.length ? (
                                    <div className="mt-2 flex flex-col gap-1">
                                        {c.attachments.map((a) => (
                                            <a key={a.id} className="text-xs underline break-all" href={a.downloadUrl} target="_blank" rel="noreferrer">
