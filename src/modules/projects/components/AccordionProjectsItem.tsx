            ...
            {!collapsed && (
                <AccordionContent className='pb-1 pl-2'>
                    <div className='mb-2 space-y-1'>
                        <NavLink to={AppRoutes.PROJECT_CREATE} onClick={onNavigate} className={({ isActive }) => cn('flex items-center gap-2 rounded-md px-3 py-1.5 text-sm hover:bg-accent/70', isActive && 'bg-accent/80')}><GitPullRequestCreate size={14} />Создать проект</NavLink>
                    </div>
                    <div className='space-y-1'>
                        {projects?.map((p) => <ProjectItem key={p.id} project={p} onNavigate={onNavigate} />)}
                    </div>
                </AccordionContent>
            )}
        </AccordionItem>
